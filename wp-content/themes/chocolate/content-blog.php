   ...
   ?> id="post-<?php the_ID(); ?>">
	  ...
	  ?>
		...
		<?php if ( SHOW_EXCERPT ) : // Only display Excerpts for archives & search ?>
			<?php
			   the_excerpt( __( '<span><i></i>Продолжить чтение</span>', 'dt' ) );
			?>
		<?php else : ?>
			<?php the_content( __( '<span><i></i>Продолжить чтение</span>', 'dt' ) );	
			?>
			<?php wp_link_pages( array( 'before' => '<div class="page-link">' . __( 'Страницы:', 'dt' ), 'after' => '</div>' ) ); ?>
		<?php endif; ?>
	  ...
	  <div class="article_footer">
		  <?php
			  ...
			  sprintf( esc_attr__( 'Показать все записи из %s', 'dt' ), get_the_author() ),
			  get_the_author()
			  );
		  ?>
		  <?php comments_popup_link( __( 'Оставить комментарии', 'dt' ), __( '1 Комментарии', 'dt' ), __( '% Комментариев', 'dt' ), 'ico_link comments' ); ?>
		  <span class="ico_link categories"><?php
			the_category( ', ' );
		  ?></span>
		  <?php the_tags( '<span class="ico_link tags">', ', ', '</span>' ); ?>
		  <?php edit_post_link( __( 'Ред.', 'dt' ), '<span class="ico_link edit-link">', '</span>' ); ?> 
	  </div>
