   ...
   ?> id="post-<?php the_ID(); ?>">
      ...
      ?>
		...
		<?php //if ( SHOW_EXCERPT ) : // Only display Excerpts for archives & search ?>
			<?php
			   //the_excerpt( __( '<span><i></i>Продолжить чтение</span>', LANGUAGE_ZONE ) );
			?>
		<?php //else : ?>
			<?php //the_content( __( '<span><i></i>Продолжить чтение</span>', LANGUAGE_ZONE ) );>

			dt_the_content( __( '<span><i></i>Продолжить чтение</span>', LANGUAGE_ZONE ) ); ?>
		 
			<?php wp_link_pages( array( 'before' => '<div class="page-link">' . __( 'Страницы:', LANGUAGE_ZONE ), 'after' => '</div>' ) ); ?>
		<?php //endif; ?>
      ...
      <div class="article_footer">
          <?php
			  ...
			  sprintf( esc_attr__( 'Показать все записи из %s', LANGUAGE_ZONE ), get_the_author() ),
			  get_the_author()
			  );
          ?>
          <?php comments_popup_link( __( 'Оставить комментарии', LANGUAGE_ZONE ), __( '1 Комментарии', LANGUAGE_ZONE ), __( '% Комментариев', LANGUAGE_ZONE ), 'ico_link comments' ); ?>
          <span class="ico_link categories"><?php
            the_category( ', ' );
          ?></span>
          <?php the_tags( '<span class="ico_link tags">', ', ', '</span>' ); ?>
          <?php edit_post_link( __( 'Ред.', LANGUAGE_ZONE ), '<span class="ico_link edit-link">', '</span>' ); ?> 
      </div>
...
<?php if (function_exists('dt_pagenavi') ) { ?>
    ...
    <ul class="paginator">
      <li class="larr"><?php next_posts_link( __( 'Старые записи', LANGUAGE_ZONE ) ); ?></div>
      <div class="rarr"><?php previous_posts_link( __( 'Новые записи', LANGUAGE_ZONE ) ); ?></div>
    </ul>
